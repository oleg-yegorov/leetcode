...
class Solution:
    ...
    def canJumpInPlace(self, nums: List[int]) -> bool:
        nums[0] = -nums[0]
        n = len(nums)
    ...
    def canJumpFaster(self, nums: List[int]) -> bool:
        n = len(nums)
...
def test_can_jump(nums: List[int], res: bool):
    assert Solution().canJumpFaster(nums) == res
